...
Cling, release 1.4. Cling is built on top of [Clang](http://clang.llvm.org) and
...
<!---Get release bugs
git log v1.3..master | grep -i "fix" | grep '#' | sed -E 's,.*\#([0-9]*).*,\[\1\]\(https://github.com/root-project/cling/issues/\1\),g' | sort
--->
...
git log --pretty=format:"%an"  v1.3...master | sort | uniq -c | sort -rn |\
  sed -E 's,^ *([0-9]+) (.*)$,\2 \(\1\),'
--->
